...
from dataLoader.KITTI_dataset3_outlier import visualize
...
class Model(nn.Module):
    ...
    def grd2cam2world2sat(self, ori_shift_u, ori_shift_v, ori_heading, level, satmap_sidelength,):
        ...
        print("yh: should not be called when combining cross-view with SLAM v6")
    ...
    def World2GrdImgPixCoordinates(self, ori_shift_u, ori_shift_v, ori_heading, XYZ_1, ori_camera_k, grd_H, grd_W,
        ...
        # print("yh: should not be called when combining cross-view with SLAM v7")


        B = ori_heading.shape[0]
        shift_u_meters = self.args.shift_range_lon * ori_shift_u           #yh here, assume ori_shift_u, ori_shift_v, ori_heading is within [-1, 1], therefore, we need to range it back
    ...
    def project_grd_to_map(self, grd_f, grd_c, shift_u, shift_v, heading, camera_k, satmap_sidelength, ori_grdH,
        ...
        XYZ_1 = self.sat2world(satmap_sidelength)  # [ sidelength,sidelength,4]   #yh: generate a world mesh (from satellite map)
        uv, mask = self.World2GrdImgPixCoordinates(shift_u, shift_v, heading, XYZ_1, camera_k,
                                                   H, W, ori_grdH, ori_grdW)  # [B, S, E, H, W,2]   # project world mesh to grd image
    ...
    def Trans_update(self, shift_u, shift_v, heading, grd_feat_proj, sat_feat):
        ...
        shift_u_new = torch.where((shift_u_new > -2) & (shift_u_new < 2), shift_u_new, rand_u)    #yh when updating network, we need to makesure the update is within [-2, 2], to make the network stable
    ...
    def corr(self, sat_map, grd_img_left, left_camera_k, gt_shift_u=None, gt_shift_v=None, gt_heading=None,
        ...
        print("yh: should not be called when combining cross-view with SLAM v1")
        ...
        for level in range(len(sat_feat_list)):
            ...
            #yh the satellite image has been rotated. The pred_u, and pred_v is in the frame of rotated satellite map
            # we need to rotate it back to the GT frame for evaluation.
    ...
    def triplet_loss(self, corr_maps, gt_shift_u, gt_shift_v, gt_heading):
        ...
        print("yh: should not be called when combining cross-view with SLAM v2")
    ...
    def CVattn_corr(self, sat_map, grd_img_left, left_camera_k, gt_shift_u=None, gt_shift_v=None, gt_heading=None,
        ...
        print("yh: should not be called when combining cross-view with SLAM v3")
    ...
    def rot_corr(self, sat_map, grd_img_left, left_camera_k, gt_shift_u=None, gt_shift_v=None, gt_heading=None,
        ...
        '''
            ...
            print("yh: should not be called when combining cross-view with SLAM v4")
    ...
    def CVattn_rot_corr(self, sat_map, grd_img_left, left_camera_k, gt_shift_u=None, gt_shift_v=None, gt_heading=None,
            ...
            for level in range(len(sat_feat_list)):
                ...
                if self.args.Optimizer == 'TransV1G2SP':
                    ...
                    grd_feat_proj, _, _, _ = self.project_grd_to_map(
                        grd_feat, None, shift_u, shift_v, heading, left_camera_k, A, ori_grdH, ori_grdW)   #yh prject grd_feat to satellite map (to obtain grd_feat_proj)
                ...
                if visualize and iter == 0:
                    g2s_feat_rot_list.append(grd_feat_proj)
                    # if level not in grd_feat_proj_dict.keys():
                    #     grd_feat_proj_dict[level] = [grd_feat_proj_]
                    #     # pred_uv_dict[level] = [sat_uv_ / torch.tensor([sat_feat.shape[-1], sat_feat.shape[-2]],
                    #     #                                          dtype=torch.float32).reshape(1,1,1,2).to(sat_feat.device)]
                    # else:
                    #     grd_feat_proj_dict[level].append(grd_feat_proj_)
                    #     # pred_uv_dict[level].append(
                        #     sat_uv_ / torch.tensor([sat_feat.shape[-1],
                        #                            sat_feat.shape[-2]], dtype=torch.float32).reshape(1,1,1,2).to(sat_feat.device))

                    # if level not in gt_uv_dict.keys():
                    #     gt_sat_feat_proj, _, gt_uv, _ = self.project_map_to_grd(
                    #         sat_feat, None, gt_shift_u, gt_shift_v, gt_heading, level)
                    #
                    #     # [B, C, H, W], [B, H, W, 2]
                    #     gt_feat_dict[level] = gt_sat_feat_proj  # [B, C, H, W]
                    #     gt_uv_dict[level] = gt_uv / torch.tensor([sat_feat.shape[-1], sat_feat.shape[-2]],
                    #                                          dtype=torch.float32).reshape(1, 1, 1, 2).to(sat_feat.device)
                        # [B, H, W, 2]
        ...
        def corr(sat_feat_list, grd_feat_list, left_camera_k, gt_shift_u=None, gt_shift_v=None, gt_heading=None,
                 pred_heading=None, mode='train', cov_i=None):
            '''
            Args:
                sat_map: [B, C, A, A] A--> sidelength
                left_camera_k: [B, 3, 3]
            ...
            # print("yh: should not be called when combining cross-view with SLAM v5")
            ...
            for level in range(len(sat_feat_list)):
                ...
                # crop_H = int(A - self.args.shift_range_lat * 3 / meter_per_pixel)
                # crop_W = int(A - self.args.shift_range_lon * 3 / meter_per_pixel)
                crop_grd = int(A - self.args.shift_range_lat * 3 / meter_per_pixel)        #yh valid range of satellite image. A: side length of satellite image,
                                                                                           # Assume the initial pose range is shifted within 1.44*2*20
                                                                                           #
                ...
                # calculate covariance circle
                # cov_i = cov_i / meter_per_pixel
                # cov_i[1] = -cov_i[1]

                xi = cov_i[0] / meter_per_pixel + corr_W / 2
                yi = (-cov_i[1] / meter_per_pixel + corr_H / 2)



                mask = polygon2mask((corr_H, corr_W), np.array([yi, xi]).T)

                if not mask.max():
                    th = 2
                    u_range = [int(corr_W / 2)-th, int(corr_W / 2)+th+1]
                    v_range = [int(corr_H / 2)-th, int(corr_H / 2)+th+1]
                ...
                mask = torch.tensor(mask).reshape(B, corr_H, corr_W).to(corr.device)
                corr_mask = torch.where(mask, corr, torch.tensor(10.0))

                max_index_m = torch.argmin(corr_mask.reshape(B, -1), dim=1)
                # corr_mask_ = np.array(corr_mask.cpu()).squeeze()
                ...
                #todo do not rotate the prediction. Then, the prediction is in grd image frame, not oxts frame.
                # cos = torch.cos(gt_heading[:, 0] * self.args.rotation_range / 180 * np.pi)
                # sin = torch.sin(gt_heading[:, 0] * self.args.rotation_range / 180 * np.pi)
                #
                # pred_u1 = pred_u * cos + pred_v * sin
                ...
                cos = torch.cos(pred_heading[:, 0] * self.args.rotation_range / 180 * np.pi)
                sin = torch.sin(pred_heading[:, 0] * self.args.rotation_range / 180 * np.pi)
            ...
            if mode == 'train':
                ...
                # return uncertainty: the last level
                corr_u = max_index % corr_W
                corr_v = max_index // corr_H
                # print(corr[0, corr_v, corr_u], corr.reshape(B, -1)[0, max_index])    corr.reshape(B, -1).min(1, keepdim=True)

                corr_ = 0.5*(2-corr)
                corr_ = corr_.reshape(B, -1)
                corr_max = torch.max(corr_)
                corr_min = torch.min(corr_)
                corr_ = (corr_-corr_min)/corr_max
                corr_p_ = corr_/(corr_.sum())
                corr_p = corr_p_.view(1, corr_W, corr_W)

                corr_p_v_axis = torch.sum(corr_p, 2)
                corr_p_u_axis = torch.sum(corr_p, 1)

                corr_w_cv = corr_p[0, corr_v, corr_u]



                # check if the cross-view is outside the covariance
                corr_u_m = max_index_m % corr_W
                corr_v_m = max_index_m // corr_H
                corr_w_m = corr_p[0, corr_v_m, corr_u_m]

                corr_w = corr_w_m/corr_w_cv


                corr_p_v = corr_p_v_axis[0, corr_v]
                corr_p_u = corr_p_u_axis[0, corr_u]


                return pred_u, pred_v, corr_p_u, corr_p_v, corr_p, corr_w, mask   #, corr_p_u_axis, corr_p_v_axis  # [B], [B]
        ...
        if mode == 'train':
            ...
            g2s_feat_list = [grd2sat8_attn, grd2sat4_attn, grd2sat2_attn]
            pred_u, pred_v, corr_u, corr_v, corr_p, corr_w, corr_m\
                = corr(sat_feat_list, g2s_feat_list, left_camera_k, gt_shift_u, gt_shift_v, gt_heading,
                                  pred_orien, mode, cov_i)    #, corr_p_u_axis, corr_p_v_axis \




            return pred_u, pred_v, pred_orien[:, 0] * self.args.rotation_range, corr_u, corr_v, corr_p, corr_w, corr_m  #, corr_p_u_axis, corr_p_v_axis
