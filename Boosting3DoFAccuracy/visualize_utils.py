...
def features_to_RGB_Yh(sat_feat, grd_feat, g2s_feat_tran, g2s_feat_rot, save_dir='./visualize/', idx=0):
    """Project a list of d-dimensional feature maps to RGB colors using PCA."""
    from sklearn.decomposition import PCA
    ...
    grd_feat = grd_feat.data.cpu().numpy()  # [B, C, H, W]
    g2s_feat_rot = g2s_feat_rot.data.cpu().numpy()
    g2s_feat_tran = g2s_feat_tran.data.cpu().numpy()
    # a list with length iters, each item has shape [B, C, H, W]
    # gt_a2g = gt_sat_feat_proj[level].data.cpu().numpy()   # [B, C, H, W]
    ...
    # flatten.append(reshape_normalize(sat_feat))
    # flatten.append(reshape_normalize(grd_feat))
    # # flatten.append(reshape_normalize(g2s_feat[:, :, H//2:, :]))
    #
    # for feat in g2s_feat:
    #     flatten.append(reshape_normalize(feat[:, :, H//2:, :]))
    #
    # flatten = np.concatenate(flatten[:1], axis=0)
    ...
    pca_sat.fit(reshape_normalize(sat_feat))

    pca_grd = PCA(n_components=3)
    pca_grd.fit(reshape_normalize(grd_feat))

    pca_g2s_rot = PCA(n_components=3)
    pca_g2s_rot.fit(reshape_normalize(g2s_feat_rot))

    pca_g2s_tran = PCA(n_components=3)
    pca_g2s_tran.fit(reshape_normalize(g2s_feat_tran))

    # for level in range(len(sat_feat_list)):
    # sat_feat = sat_feat_list[level].data.cpu().numpy()  # [B, C, H, W]
    # grd_feat = grd_feat_list[level].data.cpu().numpy()  # [B, C, H, W]
    # g2s_feat = [feat.data.cpu().numpy() for feat in pred_feat_dict[level]]
    # a list with length iters, each item has shape [B, C, H, W]
    # gt_s2g = gt_sat_feat_proj[level].data.cpu().numpy()   # [B, C, H, W]

    # B, C, A, _ = sat_feat.shape
    # B, C, H, W = grd_feat.shape
    sat_feat_new = ((normalize(pca_sat.transform(reshape_normalize(sat_feat[..., :]))) + 1 )/ 2).reshape(B, A, A, 3)
    grd_feat_new = ((normalize(pca_grd.transform(reshape_normalize(grd_feat[:, :, H//2:, :]))) + 1) / 2).reshape(B, H//2, W, 3)
    grd_feat_new_all = ((normalize(pca_grd.transform(reshape_normalize(grd_feat[..., :]))) + 1) / 2).reshape(B,H, W, 3)
    # gt_g2s_new = ((normalize(pca.transform(reshape_normalize(gt_g2s[:, :, H//2:, :]))) + 1) / 2).reshape(B, H//2, W, 3)
    g2s_rot_feat_new = ((normalize(pca_g2s_rot.transform(reshape_normalize(g2s_feat_rot[..., :]))) + 1) / 2).reshape(B, A, A, 3)
    g2s_tran_feat_new = ((normalize(pca_g2s_tran.transform(reshape_normalize(g2s_feat_tran[..., :]))) + 1) / 2).reshape(B, A, A, 3)

    sat = Image.fromarray((sat_feat_new[0] * 255).astype(np.uint8))
    sat = sat.resize((512, 512))
    sat.save(os.path.join(save_dir + '/sat_feat/sat_feat_' + str(idx).zfill(5)  + '.png'))

    # grd = Image.fromarray((grd_feat_new[0] * 255).astype(np.uint8))
    # grd = grd.resize((1024, 128))
    # grd.save(os.path.join(save_dir, '/img_feat/img_feat_' + str(idx).zfill(5)  + '.png'))

    grd_all = Image.fromarray((grd_feat_new_all[0] * 255).astype(np.uint8))
    grd_all = grd_all.resize((1024, 256))
    grd_all.save(os.path.join(save_dir + '/img_feat/img_feat_feat_' + str(idx).zfill(5) + '.png'))

    g2s_rot = Image.fromarray((g2s_rot_feat_new[0] * 255).astype(np.uint8))
    g2s_rot = g2s_rot.resize((512, 512))
    g2s_rot.save(os.path.join(save_dir + '/g2s_rot/g2s_rot_' + str(idx).zfill(5)  + '.png'))

    g2s_tran = Image.fromarray((g2s_tran_feat_new[0] * 255).astype(np.uint8))
    g2s_tran = g2s_tran.resize((512, 512))
    g2s_tran.save(os.path.join(save_dir + '/g2s_tran/g2s_tran_' + str(idx).zfill(5) + '.png'))
