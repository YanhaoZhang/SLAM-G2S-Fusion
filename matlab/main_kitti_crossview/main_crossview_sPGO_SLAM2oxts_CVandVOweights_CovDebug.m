function [vT_us0_usk, vS_0k, vM_usk_uok, Cov_circle] = main_crossview_sPGO_SLAM2oxts_CVandVOweights_CovDebug(vT_us0_usk_org, L, vT_us0_usk, vS_0k, fine_valid, vM_usk_uok, vP_usk_uok, vW_usk_uok, vMask_usk_uok, idx)
...
Cov_circle_global = zeros(num_frame, 2, num_cov_pts+1);
% Cov_circle = zeros(num_frame, 2, num_cov_pts+1);
...
if idx==0
    ...
    meter_per_pixel = 0.39165701730599667;
    kk = 21;
    % k=2;
    f_0_i0 = squeeze(Cov_circle_global(kk,:,:));
    f_k_ik = squeeze(Cov_circle(kk,:,:));
    ...
    % yi = [30, 60, 70, 30];
    % bw_size = 155;
    % BW = poly2mask(xi,yi,bw_size,bw_size);
    % imshow(BW); hold on
