function [a2t, r_slam, d_slam, r_fine, d_fine] = show_trajectory_kitti(vT_0k, slam_vTu0uk, oxts_vTu0uk, coarse_vM_ufk_upk, fine_valid)
...
if 1
fig = figure;
h_slam = plot(t_w_skw(:,1), t_w_skw(:,2), '.','Color', [0 0.4470 0.7410], 'MarkerSize', 20); hold on
h_gt = plot(t_w_okw(:,1), t_w_okw(:,2), '.', 'Color', [0.4660 0.6740 0.1880],'MarkerSize', 20); hold on
h_fine = plot(t_w_fkw(:,1), t_w_fkw(:,2), '.', 'Color',[0.6350 0.0780 0.1840],'MarkerSize', 20); hold on
...
% xlim([-319.2651  711.0131]-300)

fig = tightfig(fig);
% check the: ylim
% ylim([-650.6027  235.4278]-100)
