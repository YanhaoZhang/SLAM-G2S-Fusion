function [a2t] = show_refine_result_trajUpdate(vT_0k, slam_vTu0uk, oxts_vTu0uk, coarse_vM_ufk_upk, fine_valid)
...
%% show results
figure
% h_slam = plot(t_w_skw(:,1), t_w_skw(:,2), 'b-','LineWidth', 3); hold on
% h_gt = plot(t_w_okw(:,1), t_w_okw(:,2), '-', 'Color', [0.4660 0.6740 0.1880],'LineWidth', 3); hold on
% h_fine = plot(t_w_fkw(:,1), t_w_fkw(:,2), '-', 'Color',[0.6350 0.0780 0.1840],'LineWidth', 3); hold on
...
h_cv_pred = quiver(t_w_fkw(valid_id,1), t_w_fkw(valid_id,2), diff_t_w_oksk_cv(valid_id,1), diff_t_w_oksk_cv(valid_id,2), 'Color', 'g', 'AutoScale', 'off'); hold on
...
% legend([h_slam, h_gt, h_fine],{'SLAM', 'GT', 'SLAM+CrossView'})
axis equal
