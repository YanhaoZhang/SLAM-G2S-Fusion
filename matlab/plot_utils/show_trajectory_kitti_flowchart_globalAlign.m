function [a2t, r_slam, d_slam, r_fine, d_fine] = show_trajectory_kitti_flowchart_globalAlign(vT_0k, slam_vTu0uk, oxts_vTu0uk, coarse_vM_ufk_upk, fine_valid)
...
yaw = deg2rad(30);
R = euler_to_rotation_matrix(0, 0, yaw);
...
for k=1:num_frame
    ...
    % square root of rotation-20, 420])
% ylim([-450
...
xlabel('x (m)')
ylabel('y (m)')
% x and y limit for sequence 8:
% xlim([-20, 420])
% ylim([-450  400])

% x and y limit for sequence 2:
xlim([-300, 300])
ylim([-2150, 10])
% colormap('jet')
colorbar;
