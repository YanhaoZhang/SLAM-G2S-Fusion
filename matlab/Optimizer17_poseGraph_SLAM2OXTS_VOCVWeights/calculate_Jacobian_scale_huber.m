function [J] = calculate_Jacobian_scale_huber(vT_0k, vS_0k, vo_vM_usl_usk, pred_vR_usk_uok, pred_vM_usk_uok, w)
...
num_obs = nVO + nCV;
dim_vo = dim_r+dim_t;
J = zeros(dim_vo*nVO + dim_r*nCV_r + dim_t*nCV + dim_s*(nPoses-1),   (dim_r+dim_t+dim_s)*nPoses);    % [r t scale] * nPoses
...
for a=1:nCV
    ...
    % J(dim_vo*nVO+dim_vo*(id_o-1)+(1:dim_r),       (dim_vo+dim_s)*(k-1)+(1:dim_r)) =  Jer_Jrk;
...
J(dim_vo*nVO+dim_r*nCV_r+dim_t*nCV+dim_s*(id_o-1)+(1:dim_s), (dim_vo+dim_s)*(l-1)+dim_r+dim_t+(1:dim_s)) = Jes_Jsl;
J(dim_vo*nVO+dim_r*nCV_r+dim_t*nCV+dim_s*(id_o-1)+(1:dim_s), (dim_vo+dim_s)*(k-1)+dim_r+dim_t+(1:dim_s)) = Jes_Jsk;
