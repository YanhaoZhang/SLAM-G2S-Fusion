function [F] = calculate_Error_scale_huber( vT_0k, vS_0k, vo_vM_usl_usk, pred_vR_usk_uok, pred_vM_usk_uok, w )
...
% num_obs = nVO + nCV;
dim_vo = dim_r+dim_t;
F = zeros( dim_vo*nVO + dim_r*nCV_r + dim_t*nCV + dim_s*(nPoses-1), 1);    % initialize F
...
F(dim_vo*nVO+dim_r*nCV_r+dim_t*nCV+dim_s*(id_o-1)+(1:dim_s),1) = [es];
id_o = id_o+1;
